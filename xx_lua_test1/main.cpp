...
//    }
    ...
    {
        xx::Lua::SetGlobal(L, "xxx", [](int const &a, int const &b) {
            return a + b;
        });
        ...
        xx::Lua::DoString(L, R"===(
local a = 1.23
local r = xxx(a, a) -- lua 5.4.4 need set LUA_FLOORN2I F2Ifloor macro compile
print(r)
    )===");
            })) {
    ...
    if (auto top = lua_gettop(L)) {
        std::cout << "top = " << top << std::endl;
        for (int i = 1; i <= top; ++i) {
            auto typeId = lua_type(L, i);
            auto typeName = lua_typename(L, typeId);
            std::cout << "L[" << i << "]'s type = " << typeName << std::endl;
            switch (typeId + 1) {
            case 0:
            ...
            case 1:
                ...
                break;  // nil
            case 2:
                std::cout << "value = (bool)" << xx::Lua::To<bool>(L, i) << std::endl;
                break;  // boolean
            case 3:
                std::cout << "value = (userdata)" << (size_t)xx::Lua::To<void*>(L, i) << std::endl;
                break;  // userdata
            case 4:
                std::cout << "value = (number)" << xx::Lua::To<double>(L, i) << std::endl;
                break;  // number
            case 5:
                std::cout << "value = (string)" << xx::Lua::To<std::string_view>(L, i) << std::endl;
                break;  // string
            case 6:
            ...
            case 9:
                ...
                break;  // thread
            case 10:
                std::cout << "value = upvalue" << std::endl;
                break;  // upvalue
            case 11:
                std::cout << "value = proto" << std::endl;
                break;  // proto
            }
        }
    }
