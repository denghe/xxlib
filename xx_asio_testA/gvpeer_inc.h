#pragma once
// 公共代码。 在 Server 后 include。之后 自已实现     VPeer::ReceiveRequest    GPeer::HandleCommand_Accept
// Server 需要有 .ioc  .om  .gpeers  成员
...
struct GPeer : xx::PeerCode<GPeer>, xx::PeerTimeoutCode<GPeer>, xx::PeerHandleMessageCode<GPeer>, std::enable_shared_from_this<GPeer> {
    ...
    int HandleData(xx::Data_r&& dr) {
        ...
        if (target != 0xFFFFFFFFu) {
            ...
            if (cmd == "accept"sv) {                                                // gateway accept client socket
                ...
                if (int r = HandleCommand_Accept(ip)) return -__LINE__;             // ip 前置 check
    ...
    // 内部指令 accept 的处理函数
    int HandleCommand_Accept(std::string_view ip);
...
inline void VPeer::RemoveFromOwner() {
    assert(ownerPeer);
